...
Install lines rot in a specific way: the distribution gets renamed or published,
and the instruction keeps pointing at a git URL or an old name. Nothing fails —
the docs are not executed — and the person who finds out is a stranger whose
very first command errors. Both happened here: a sibling file told readers to
`pip install eval-gate`, which does not exist, and this README kept a git URL
and a stale "once on PyPI" note long after the package was on PyPI.
